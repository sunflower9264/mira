...
- `PromptTokenEditor` 把系统工具、Skill、MCP，以及当前节点和直接上游 JSON Schema 中的字段/枚举值显示为标签；存储、复制和提交仍保留原始字符串，标签按整体删除，下拉选择后插入当前光标位置。字段中文名和说明只读取 Schema 的 `title` / `description`，不按标识符猜译，也不建立运行时字段绑定。
