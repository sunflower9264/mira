...
- `PromptTokenEditor` 把系统工具、Skill、MCP，以及当前节点和直接上游的可引用结果显示为标签；存储、复制和提交仍保留内部字符串，标签按整体删除，下拉选择后插入当前光标位置。结果名称和说明只读取 Schema 的 `title` / `description`，界面不展示 property key 或 JSON 路径，不按标识符猜译，也不建立运行时字段绑定。
