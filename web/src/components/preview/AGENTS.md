...
- live SSE 收到或重放 `run.resumed` 时必须清除已回答问题并恢复 `running`；桌面和 Mobile 共用 `useRunStore`，不要在页面组件各自维护等待状态。
- StepTab 的“生成提示词”调用 Prompt Assistant API，只提交用户说明而不传节点 model/reasoning_effort；完成后把 prompt/可选 output_contract 作为一次可撤销 graph 变更直接写回。生成态按节点 id 存在 editor store，切换节点不自动取消。
