...
function WorkspaceShell({ workspace, mobile = false, onBack }: { workspace: Workspace; mobile?: boolean; onBack(): void }) {
  ...
  return (
    <div className="min-h-full bg-[#F4F5F7] text-[#0B0B0F]">
      {!mobile ? <TopNav /> : null}
      <main className={`${mobile ? 'px-4 pb-8 pt-4' : 'mx-auto max-w-7xl px-6 pb-12 pt-8'}`}>
        <div className="mb-5 flex items-center gap-3">
          ...
          <div className="min-w-0 flex-1"><h1 className="truncate text-2xl font-semibold tracking-tight">{workspace.name}</h1><p className="mt-0.5 truncate text-xs text-black/45">{workspace.description || '持久 Codex 项目工作空间'}</p></div>
        ...
        <div className={`${mobile ? 'flex flex-col gap-4' : 'grid grid-cols-[220px_minmax(0,1fr)] gap-5'}`}>
          <aside className="rounded-[22px] border border-black/5 bg-white p-3 shadow-card">
            ...
            {sessions.length === 0 ? <div className="px-2 py-4 text-xs leading-5 text-black/40">创建一个会话，让 Codex 记住这个项目的上下文。</div> : null}
          </aside>
          <section className="min-w-0 rounded-[22px] border border-black/5 bg-white shadow-card">
            <div className="flex gap-1 overflow-x-auto border-b border-black/5 p-2">{([['chat','Codex'],['files','文件'],['workflow','工作流'],['git','Git']] as const).map(([key,label]) => <button type="button" key={key} onClick={() => setTab(key)} className={`rounded-full px-3 py-2 text-sm ${tab === key ? 'bg-black text-white' : 'text-black/55 hover:bg-black/5'}`}>{label}</button>)}<button type="button" onClick={() => void refresh()} className="ml-auto grid h-9 w-9 shrink-0 place-items-center rounded-full text-black/40 hover:bg-black/5" aria-label="刷新"><RefreshCw className={`h-4 w-4 ${refreshing ? 'animate-spin' : ''}`} /></button></div>
      ...
      <PromptDialog open={renameSessionOpen} onClose={() => setRenameSessionOpen(false)} onConfirm={renameSession} title="重命名会话" description="Codex thread 与项目上下文不会改变。" inputLabel="会话名称" value={sessionTitle} onChange={setSessionTitle} disabled={!sessionTitle.trim() || sessionTitle.trim() === session?.title} />
...
function WorkspaceChat({ workspace, session, events, onEvents, onSessionUpdate, onNew, onDelete, onRename }: { workspace: Workspace; session: WorkspaceSession | null; events: WorkspaceEvent[]; onEvents(next: WorkspaceEvent[]): void; onSessionUpdate(next: WorkspaceSession): void; onNew(): void; onDelete(): void; onRename(): void }) {
  ...
  if (!session) return <div className="flex min-h-80 flex-col items-center justify-center text-center"><Bot className="h-8 w-8 text-black/20" /><div className="mt-3 text-sm font-medium text-black/65">选择或创建一个会话</div><div className="mt-1 text-xs text-black/40">每个工作空间可以保留多个独立的 Codex thread。</div></div>;
  const send = async () => {
  ...
  return <div className="flex min-h-[520px] flex-col"><div className="mb-4 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between"><div><h2 className="text-base font-semibold">{session.title}</h2><p className="mt-0.5 text-xs text-black/40">{session.thread_id ? `Codex thread 已连接 · ${model || '默认模型'}` : '尚未开始对话'}</p></div><div className="flex w-full items-center justify-end gap-1 sm:w-auto"><button type="button" onClick={() => setToolsOpen(true)} className="rounded-full px-3 py-1.5 text-xs text-black/45 hover:bg-black/5 hover:text-black">会话工具</button><button type="button" onClick={onRename} className="rounded-full px-3 py-1.5 text-xs text-black/45 hover:bg-black/5 hover:text-black">重命名</button><button type="button" onClick={onDelete} className="rounded-full px-3 py-1.5 text-xs text-red-500 hover:bg-red-50">删除</button></div></div>{actionStatus ? <div className="mb-3 rounded-xl border border-black/5 bg-black/[0.025] px-3 py-2 text-xs text-black/55">{actionStatus}</div> : null}<div ref={messagesRef} className="min-h-64 max-h-[55dvh] flex-1 space-y-3 overflow-y-auto rounded-2xl bg-[#F4F5F7] p-3">{events.length === 0 ? <div className="flex h-64 items-center justify-center text-xs text-black/35">告诉 Codex 你想在这个项目中完成什么。</div> : collapseWorkspaceEvents(events).map((event) => <EventBubble key={event.id} event={event} />)}{waiting ? <DecisionPromptPanel context={waiting.request.context} groups={waiting.request.groups} disabled={sending} onComplete={(answers) => void submitDecision(answers)} /> : null}</div><div className="mt-3 rounded-2xl border border-black/10 bg-white p-2 shadow-pill">{attachments.length ? <div className="flex flex-wrap gap-1.5 px-2 pt-1">{attachments.map((item) => <button type="button" key={item.id} onClick={() => setAttachments((current) => current.filter((entry) => entry.id !== item.id))} className="inline-flex items-center gap-1 rounded-full bg-black/[0.05] px-2.5 py-1 text-[11px] text-black/60">{item.name}<X className="h-3 w-3" /></button>)}</div> : null}<textarea value={text} onChange={(event) => setText(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter' && !event.shiftKey) { event.preventDefault(); void send(); } }} placeholder="向 Codex 描述下一步… 支持 /new、/compact、/review、/model、/status（不提供 /diff）" className="min-h-16 w-full resize-none bg-transparent px-2 py-1.5 text-sm leading-6 outline-none placeholder:text-black/35" disabled={isRunning || waiting !== null} /><div className="flex items-center justify-between px-1"><span className="text-[11px] text-black/35">同一工作空间一次只执行一个 turn</span><div className="flex items-center gap-1"><label className={`grid h-8 w-8 place-items-center rounded-full text-black/35 hover:bg-black/5 ${isRunning ? 'pointer-events-none opacity-30' : 'cursor-pointer'}`} aria-label="添加附件"><Paperclip className="h-4 w-4" /><input type="file" multiple className="hidden" onChange={(event) => void uploadAttachments(event)} /></label>{isRunning && turnId ? <button type="button" onClick={() => void api.interruptWorkspaceTurn(turnId)} className="grid h-8 w-8 place-items-center rounded-full text-red-500 hover:bg-red-50" aria-label="停止"><Square className="h-3.5 w-3.5 fill-current" /></button> : <button type="button" onClick={() => void send()} disabled={!text.trim() || uploading || waiting !== null} className="grid h-8 w-8 place-items-center rounded-full bg-black text-white hover:bg-black/85 disabled:opacity-30" aria-label="发送"><Send className="h-4 w-4" /></button>}</div></div></div><SessionToolsDialog open={toolsOpen} onClose={() => setToolsOpen(false)} workspace={workspace} session={session} models={models} model={model} capabilities={capabilities} onModel={setModel} onStatus={setActionStatus} /></div>;
}
...
function WorkspaceWorkflow({ workspace, proposals, onRefresh }: { workspace: Workspace; proposals: WorkspaceWorkflowProposal[]; onRefresh(): void }) {
  ...
  return (
    <div>
      <div className="mb-4"><h2 className="text-base font-semibold">可视化工作流</h2><p className="mt-0.5 text-xs leading-5 text-black/40">Codex 提出的图变更先经过 lint，再由你确认写入正式 App；这里只读预览。</p></div>
      ...
      <AppDialog open={activeRunApp !== null} onClose={() => { useRunStore.getState().reset(); setActiveRunApp(null); }} title={activeRunApp?.name ?? '运行应用'} description="运行继续使用 Mira 正式链路；中段提问、停止、输出和文件都在此处理。" widthClassName="max-w-5xl">
