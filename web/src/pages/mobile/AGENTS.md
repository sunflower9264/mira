...
- 最终结果展示 output HTML 和 run artifacts；文件下载只使用后端返回的 `download_url`。`ResultView` 复用同一份 `useRunArtifacts` 状态渲染文件列表并传给 `HtmlOutputFrame`，只允许同名 `verified` artifact 绑定 HTML 中的下载或图片预览占位。
