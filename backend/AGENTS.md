...
- Claude/Codex CLI 只能通过 `app/runtime/sandbox.py` 在 Docker Linux sandbox 容器内执行；不要恢复宿主机 `subprocess.Popen` 直跑 Agent CLI。Office artifact 深检是后端宿主机上的独立确定性校验，不在 Agent runtime 内执行。
- sandbox 内置截图工具 `/opt/mira/capture_screenshots.py`；目标项目有 `package-lock.json` 时必须执行 `npm ci`，否则执行 `npm install`，并在启动前依次执行项目已声明的 `db:init`、`db:seed` 脚本。Chromium 的 HOME、XDG 目录和 `--user-data-dir` 使用截图调用结束即清理的临时目录。每个 route 在 Chromium 前必须通过最终状态 `<400` 的 HTTP 检查；`--min-screenshots` 默认 1，截图不足、HTTP 错误或其他 capture failure 时 manifest `ok=false` 且 CLI 非零退出。manifest/log 只保留相对路径或占位路径并保持 URL 原样；ZIP 解包额外拒绝规范化后重名成员和损坏压缩流，ZIP/TAR 都拒绝 traversal、链接与特殊文件，并限制 10,000 个成员和 1 GiB 总展开量。
...
- `generate.output_contract` 支持 json/html/artifact；普通 generate 默认自由文本且不设置契约。只有下游需要稳定字段、当前节点明确输出 HTML 片段或需要可下载文件产物时才使用契约。JSON 必须提供 strict object `json_schema`，artifact 必须提供 `artifact_kind` 且只接受运行工作区内文件 `path`；`zip` kind 只接受真实有效的 `.zip`。可选 bool `validate_office_documents` 只用于 artifact，且只允许 `docx`、`excel`、`ppt`、`zip`、`file`；设为 `true` 时要求直接产物或 ZIP 内至少有一个 Office 文档，并由宿主机 LibreOffice 和 `pdfinfo` 验证可打开且页数大于 0。深检最多并发 2 个、总时限 120 秒、响应 run cancel；必须通过 system manager transient unit、专用无 Docker 组账号和 root-owned helper 隐藏 HOME/运行时 socket、限制网络和资源。helper、ACL 或宿主机工具缺失属于不可修复基础设施错误，不得退回仅 `prlimit` 的非隔离进程，也不调用 Agent repair。JSON、HTML、自由文本和 artifact 文本/OOXML 成员都拒绝 `U+FFFD`；带 `.zip`、OOXML、`.tar`、`.gz` 或 `.tgz` 扩展名的文件必须是对应有效容器，ZIP/TAR/OOXML 扫描拒绝 traversal、链接和特殊文件，并限制 10,000 个成员、64 MiB 文本/XML、512 MiB 压缩文件与 1 GiB 总展开量。成功 Step 将 artifact 输出归一化为带相对路径、大小、SHA-256、artifact kind 和版本号的 manifest；Run 写入 success 前复验所有新版 manifest，文件缺失、manifest 非法或大小/hash 不符必须使 Run 失败，缺少版本号的旧声明保持兼容；成功终态必须使用条件更新，不能覆盖并发取消。`output` 节点保持 HTML-only，并内部使用 `{"html":"..."}` wrapper 契约。
...
- Run Files 和 Step Trace 按需从成功 artifact contract Step 的声明产物组装，不扫描 workspace，也不新增 artifact 表。响应必须返回 `sha256` 和 `integrity`（`verified` / `modified` / `legacy_unverified`）以及 hash-bound 签名下载链接，禁止泄漏 runtime 本地绝对路径；`modified` 产物不可下载。
