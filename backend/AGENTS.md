...
- 一次 Run 只创建一个逻辑 RunAgent。线性节点共享 provider session 和 branch workspace；只有真实 fan-out 才从 checkpoint 原生 fork session 并创建 CoW workspace，fan-in 由协调 Agent 基于完整 branch snapshot/context 合并，receipt 通过路径、来源与 hash 校验后才消费分支。`/mnt/inputs` 仅用于当前 ask_user 恢复附件；不得恢复 `/mnt/results` 或每节点独立 workspace/session。
