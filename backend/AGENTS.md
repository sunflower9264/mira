...
- 一次 Run 只创建一个逻辑 RunAgent。线性节点共享 provider session 和 branch workspace；只有真实 fan-out 才从 checkpoint 原生 fork session 并创建 CoW workspace，fan-in 由协调 Agent 基于完整 branch snapshot/context 合并，receipt 通过路径、来源与 hash 校验后才消费分支。Application Run 的 `/mnt/inputs` 仅用于当前 ask_user 恢复附件；不得恢复 `/mnt/results` 或每节点独立 workspace/session。
...
- Rerun-from 是 checkpoint rerun：必须创建新 run，冻结 cut 前的 workspace checkpoint、provider session lineage 和 step 状态，使用当前 App graph 只执行 cut 及后代。当前 Graph 新增的 cut 前节点标为 `checkpoint_reused`，cut 前 input override 被忽略；旧 run 只读，没有有效 pre-checkpoint 时返回 409。
