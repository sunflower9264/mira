...
`scripts/` 包含开发启动、环境初始化、管理员初始化、普通用户维护、runtime 镜像检查、内部 ask_user MCP helper 和真实 runtime smoke 验证脚本。
...
- `init_env.py` 不覆盖已有 secret；首次创建 `.env` 时填充 generated placeholders，已有 `.env` 时当前只补齐或替换 `AGENT_CONFIG_SECRET`。
- `init_admin.py` 从 `.env` 读取 `ADMIN_USERNAME` / `ADMIN_PASSWORD`，校验后 upsert 固定管理员并初始化全局 settings。
- `ask_user_mcp_server.py` 是容器内内部 ask_user MCP helper；协议变化必须同步 runtime bridge、run resume、decision schema 和相关 prompt templates，ask_user input schema 必须要求 context/title/summary。
