...
- generate `JWT_SECRET` and `CODEX_CONFIG_SECRET` placeholders;
...
app/
  ...
  runtime/    AgentRuntime interface, Codex App Server adapter, Docker sandbox runner
migrations/   Alembic migration environment and versions
scripts/      dev, init, smoke, and admin scripts
seeds/        gallery and prompt template seeds
...
- Each user also owns one long-lived Wiki. Raw sources are versioned separately from workflow uploads; the Maintainer produces Markdown under `wiki/`, and every new Run freezes an immutable read-only snapshot. No chunking, embeddings, vector database, or RAG retrieval is used.
...
- Wiki ingestion is persisted in `wiki_operations`. Convertible documents are parsed with pinned MarkItDown inside a network-disabled runtime helper, then the Codex Maintainer updates a candidate tree. A failed operation does not replace the current revision. Archives and other non-convertible formats are rejected on upload and rename.
- Settings, Skills metadata, MCP config, Codex config/auth, supported models, global instructions, and Prompt Templates are global and admin-maintained. Admins can preview uploaded Skill archives through the stored `SKILL.md` without injecting the Skill into shared fake HOME.
- A Skill may declare `requirements.lock` or `requirements.txt` beside its unique `SKILL.md`; the lock file wins. Mira builds binary-wheel-only Python dependencies in isolated, credential-free containers, caches the verified layer for the current runtime image, and mounts it read-only at that Skill root's `.deps`. Skill code must add its own `.deps` to `sys.path`; operating-system packages and executables still belong in the runtime image. Archives containing a managed `.deps` directory are rejected.
- Codex `config.toml` and `auth.json` contents are stored encrypted in the database. Runtime files under scoped fake HOME directories are regenerated from DB and may overwrite local edits.
- Enabled MCP servers and Skills form the global Tool inventory. Apps use all enabled Tools by default and can exclude individual Tools through `graph.tools.disabled_tool_ids`; LLM graph nodes do not use `allowed_tools` to enable or disable Tools. MCP servers and Skills also have a `planning_enabled` flag, defaulting to false; only flagged tools are injected into NL compile, Prompt Assistant, and app-run planning/read-only calls.
- Supported models live in Settings as one Codex list. They are manually maintained by admins and required when saving Codex config; models are not inferred from config text, App Server state, or auth files.
- Keep `CODEX_CONFIG_SECRET` with database backups; encrypted Codex config cannot be decrypted without it.

## Agent Runtime

Codex (`@openai/codex`) runs as an App Server only inside the Docker Linux sandbox image built from `backend/runtime/Dockerfile`. The image keeps Python 3.12 and Node.js 22. `scripts/ensure_runtimes.py`, which runs as part of `scripts/dev.py` and the root startup scripts, builds the image when Docker is available and rebuilds it when the Dockerfile or bundled runtime helpers change. The backend never executes Codex on the host. Runtime containers enable Docker init so descendant processes from Codex, Chromium and development servers are reaped. The runtime image includes Chromium, CJK fonts and `/opt/mira/capture_screenshots.py`; Office artifact validation remains outside the image to keep it small. The screenshot helper calls `/usr/bin/chromium` directly so a workspace PATH wrapper cannot replace the browser. It accepts only a project inside the current workspace through `--project-dir`, requires an existing `node_modules`, and never installs frontend dependencies. Before starting the dev server it runs declared `db:init` and `db:seed` scripts in that order when present. Each capture receives a temporary Chromium HOME/XDG/user-data profile that is removed afterward. Each route must return a final HTTP status below 400 before Chromium runs. `--min-screenshots` defaults to 1. The screenshot ZIP contains only PNG files, its manifest, and its log, and is created only when `manifest.ok=true`; it does not package project source. An unmet threshold, HTTP error, or other capture failure returns a non-zero exit code and removes any stale ZIP at the requested output path. Failure diagnostics remain in the output directory's manifest and log. The manifest and log redact runtime filesystem roots with diagnostic placeholders while preserving URLs.

The image also pins `@playwright/cli` and exposes it through `mira-browser`, which binds the executable to `/usr/bin/chromium`, headless isolated mode, and a runtime-owned empty config. Interactive browser evidence must run `mira-browser doctor` first and then use the same entry point for `open`, `snapshot`, interactions, and screenshots. Runtime `npx`, package installation, browser downloads, alternate browser/config/profile/CDP endpoints, and headed mode are rejected. This interactive entry point remains separate from the deterministic route-batch helper `/opt/mira/capture_screenshots.py`.
...
- Each runtime turn starts a short-lived App Server container from `RUNTIME_SANDBOX_IMAGE`; thread identity persists independently of that process.
- The current branch workspace is mounted read-write at `/workspace` for execution and read-only for planning; scoped HOME is mounted read-write at `/home/mira`; current input attachments are staged read-only at `/mnt/inputs`; a frozen Run Wiki, when present, is mounted read-only at `/mnt/wiki`.
...
- Third-party Apps cannot read a runner's Wiki until the runner grants access for the exact `user_id + app_id + normalized graph SHA-256`. A runner may decline and run without Wiki; graph changes invalidate the remembered grant.
...
- Codex reads scoped `config.toml` and `auth.json` copied from encrypted DB-derived shared config. Snapshot-allowed MCP servers are written into scoped config, not passed through argv.
- Host-level Codex login state is not used by backend runs.
- Runtime env is configured through `RUNTIME_SANDBOX_IMAGE`, `RUNTIME_DOCKER_NETWORK`, `RUNTIME_CONTAINER_MEMORY`, `RUNTIME_CONTAINER_CPUS`, and `RUNTIME_CONTAINER_PIDS_LIMIT`.
...
- The adapter initializes App Server JSON-RPC, starts/resumes/forks a Codex thread, then starts a turn. `turn/completed` ends the short-lived container; streaming item notifications are converted to the existing Step delta contract.
- Planning uses `collaborationMode=plan`; the outer Docker sandbox mounts the branch workspace read-only and the App Server turn declares `externalSandbox` with restricted network access, avoiding an unsupported nested Linux sandbox. Native `item/tool/requestUserInput` requests are normalized into Mira waiting events; the browser answer is returned with the original JSON-RPC request id so the same turn continues. User questions use this native transport exclusively and are not routed by prompt text. They only collect information the current node can directly use: returning to an upstream node, changing Graph control flow, restarting a run, or retrying a tool failure remains the separate checkpoint-rerun path. A planning result that still reports missing user decisions without issuing a native request is retried once and then fails as a contract error instead of entering node execution.
...
- `nlcompile_plan`: asks Codex to produce a structured confirmation plan with goal, assumptions, data flow, implementation steps, graph changes, expected inputs, expected outputs, and acceptance criteria. New workflows and large edits do not trigger questions by themselves; missing material business decisions can be requested through App Server's native `requestUserInput`. Key implementation, input, output, change, and acceptance lists must be non-empty so structurally valid but content-free plans enter repair instead of reaching confirmation.
- `nlcompile_graph_patch`: after the user confirms a plan, asks Codex to produce at least one graph patch from `$confirmed_plan`. The shared patch protocol includes the legal JSON shape for every node type plus normal and condition edges, so empty graphs do not require the model to guess wire fields. This stage must not request more user input, redesign the plan, return an empty success, or change topology for visual layout; a later layout pass owns coordinates. Added nodes receive deterministic fallback coordinates before layout so a layout failure still returns an editable graph.
...
- `output_contract_repair`: rewrites a generate node result once when it fails the configured `output_contract`. The repair receives the original task context and must preserve facts, values, order, language, paths, and existing HTML while making only the minimum structural correction. When U+FFFD is detected, Mira marks damaged spans, starts a fresh Codex thread, and requires complete regeneration of the affected field or sentence; it never accepts deletion-only cleanup.
- `prompt_assistant`: instructs Codex to first decide whether the user's request is a new node goal or an edit to the existing prompt, then generate a new prompt or minimally update the current generate/condition/output prompt using direct upstream/downstream graph context. The target prompt is passed in full, related prompts preserve both head and tail, and the current generate `output_contract` is visible; contexts over 200 KiB fail explicitly instead of silently truncating constraints. Button calls may issue one focused native `requestUserInput`; NL compile apply-time post-processing uses the same template but does not enter user interaction.
- `status_smoke`: verifies that Codex can complete a short real call.

NL compile is two-stage and persisted in `nlcompile_sessions`. `POST /api/nlcompile` returns `planned` with `compile_id`, structured `plan`, and `plan_markdown`; it does not return `new_graph` or `applied_patches`. `GET /api/apps/{app_id}/nlcompile/active` returns the latest active session for restore, or `204 No Content` when there is no active session, and `POST /api/nlcompile/{compile_id}/refine` reruns the plan stage with structured history and user feedback while preserving the same compile id. `POST /api/nlcompile/{compile_id}/apply` uses the saved current graph, confirmed plan, and history to generate patches. Apply treats the Codex output as a batch: the backend simulates the full patch set on a temporary graph, retries order-dependent `update_node` and `add_edge` patches after later patches have been applied, then runs full graph structure, prompt-node, topology, and NL-compile-only redundant-transitive-edge validation. When the batch adds or updates prompt nodes, NL compile calls Prompt Assistant to rewrite those prompts with the confirmed plan and question history before applying AI layout beautification. If parsing, patch application, prompt-assistant post-processing, or validation fails, the backend asks Codex to regenerate the complete `{"patches":[...]}` JSON up to 3 total attempts. If repair still fails, the request returns 502 and no partially applied graph is returned. Backend startup marks planning, waiting, and applying nlcompile sessions as `interrupted` so the frontend can restore or replay from DB state.
