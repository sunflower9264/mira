...
- `GET /api/runs/{run_id}/artifacts` returns a run-level artifact list for the run owner. It is assembled on demand only from successful artifact output-contract Steps, never by scanning the run workspace, and does not require a separate artifact table. Responses include the manifest SHA-256 and `integrity` (`verified`, `modified`, or `legacy_unverified`) with hash-bound signed download URLs. New manifests record relative path, size, SHA-256, artifact kind, and manifest version; before a Run is marked successful, all new versioned manifests are revalidated, and a missing file, invalid manifest, or size/hash change fails the Run. The final success write is conditional so a concurrent cancellation cannot be overwritten. Legacy declarations remain readable as `legacy_unverified` and are not part of that finalization revalidation; changed or invalid versioned declarations take precedence over same-path legacy entries and cannot be downloaded.
