...
- Wiki ingestion is persisted in `wiki_operations`. Convertible documents are parsed with pinned MarkItDown inside a network-disabled runtime helper, then the Codex Maintainer updates a candidate tree. A failed operation does not replace the current revision. Unsupported formats remain downloadable raw files.
