...
- The current branch workspace is mounted read-write at `/workspace`; scoped HOME is mounted read-write at `/home/mira`; current input attachments are staged read-only at `/mnt/inputs`.
...
- Planning uses `collaborationMode=plan` with a read-only sandbox policy. Native `item/tool/requestUserInput` requests are normalized into Mira waiting events; the browser answer is returned with the original JSON-RPC request id so the same turn continues. User questions use this native transport exclusively and are not routed by prompt text.
