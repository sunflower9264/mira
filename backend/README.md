...
- Run execution is dependency-driven. Sequential LLM nodes on one branch resume the same Codex thread and writable workspace. A true fan-out forks the parent thread with App Server `thread/fork` and creates a CoW branch workspace; fan-in uses a coordinating Codex branch and validated merge receipt. Node Envelopes and output contracts remain the formal result boundary even though later nodes can use shared thread/workspace context.
