...
- The current branch workspace is mounted read-write at `/workspace` for execution and read-only for planning; scoped HOME is mounted read-write at `/home/mira`; current input attachments are staged read-only at `/mnt/inputs`.
