...
def test_prompt_templates_are_seeded_and_saved_to_seed(auth_client):
    ...
    assert "修改当前提示词" in items["prompt_assistant"]["content"]
    assert "未涉及内容必须原样保留" in items["prompt_assistant"]["content"]
    assert "不得遗漏、概括或弱化" in items["prompt_assistant"]["content"]
    assert "不要把修改指令本身写进最终 prompt" in items["prompt_assistant"]["content"]
    assert "推荐、选择、排序、个性化类 generate 节点" in items["prompt_assistant"]["content"]
    assert "信息足够时不要确认" in items["prompt_assistant"]["content"]
