...
def _build_planning_prompt(prompt: str) -> str:
    return "\n\n".join(
        [
            ...
            "只询问当前节点可以在本次执行中直接采用的用户信息、偏好或约束。",
            "不得把重新执行上游节点、跳转节点、改变执行分支、重启运行，或重试工具与基础设施错误包装成用户选项；原生提问的回答只会恢复当前节点，不会改变 Graph 调度。",
            "上游产物缺失、业务门禁未通过或工具执行失败不是用户选择可以消除的决策；此时不要提问，继续当前节点并按其输出契约返回准确的失败或阻断结果。真正的节点重新执行由运行结束后的 checkpoint rerun 创建新 Run。",
