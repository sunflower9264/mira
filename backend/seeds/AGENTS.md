...
`seeds/` 保存初始化数据：内置 gallery 模板、默认 Agent provider 元数据、prompt template 默认内容和 gallery 封面素材。
...
- `agents.json` 维护默认 Agent provider 元数据；supported models 由管理员在 Settings 中维护，不在 seed 中硬编码真实可用模型列表。
...
- `ask_user_protocol` 与 `ask_user_preflight_protocol` 职责不同：前者是真实 ask_user 工具协议，后者是运行期 preflight JSON 状态机，不要混用；两者都必须要求 ask_user request 带 `context.title` 和 `context.summary`。
